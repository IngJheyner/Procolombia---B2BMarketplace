...
function create_product($item) {
  ...
  if ($nid_company > 0) {
    ...
    if (!$exist_product) {
      ...
      addMessage('No se creo el producto: ' . $item['A'] . ' - ' . $item['M'] . ', ya esta creado en la plataforma', 'warning');
    }
  }
  else {
    addMessage('No se creo el producto: ' . $item['A'] . ' - ' . $item['M'] . ', no existe la empresa con id: ' . $item['E'], 'warning');
  }
}
...
function getImage ($url, $directory, $data = []) {
  ...
  if (!empty($url)) {
    ...
    $file_uri = normalizeUri($destination . '/' . $image_name);
    $file =  file_save_data($imageString, $file_uri, $replace = \Drupal\Core\File\FileSystemInterface::EXISTS_RENAME);
    if ($file->id()) {
      $image_tax = [
        'target_id' => $file->id(),

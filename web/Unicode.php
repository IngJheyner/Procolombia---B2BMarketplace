...
else if (get_class($text) !="Drupal\Core\Link"){
if (strlen($text) == 0) {
      return TRUE;
    }
    // With the PCRE_UTF8 modifier 'u', preg_match() fails silently on s$
    // containing invalid UTF-8 byte sequences. It does not reject chara$
    // codes above U+10FFFF (represented by 4 or more octets), though.
    return (preg_match('/^./us', $text) == 1);
